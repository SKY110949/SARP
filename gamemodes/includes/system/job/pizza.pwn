...
hook OnGameModeInit()
{
	...
	jobtext = Create3DTextLabel("{FF0000}Pizzaboy Job\n{FFFFFF}ใช้ {7FFF00}/startjob{FFFFFF} เพื่อเริ่มงาน!",0x008080FF,2104.2502,-1806.3750,13.5547,30.0,0,1);
	restocktext = Create3DTextLabel("{FF0000}Restocking Point\n{FFFFFF}ใช้ {7FFF00}/restock{FFFFFF} เพื่อรับกล่องพิซซ่าใส่มอไซต์!",0x008080FF,2116.4973,-1788.3663,13.1152,30.0,0,1);
...
hook OnPlayerDeath(playerid, killerid, reason)
{
	if(IsInJob[playerid] == 1)
	{
		...
		HidePlayerPizzaBikeTexts(playerid);
		HideTipTimeText(playerid);
		SendClientMessage(playerid,-1," คุณตายและงาน part-time ถูกยกเลิก!");
	}
	return 1;
}
...
{
	SendClientMessage(playerid,-1," คำสั่งพิซซ่า: /startjob, /endjob, /restock, /getpizza, /putbackpizza");
	return 1;
}
	    ...
	    if(IsInJob[playerid] == 1)
	    {
	        if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	        {
	            if(PizzaBikesPizzas[GetPlayerVehicleID(playerid)] == 0)
	            {
	                ...
	                SendClientMessage(playerid,-1," คุณนำพิซซ่าใส่รถมอไซต์ {FF0000}5 {FFFFFF}กล่อง!");
	            ...
	            else return SendClientMessage(playerid,-1," มอไซต์ของคุณยังมีพิซซ่าเหลืออยู่!");
	        }
	        else return SendClientMessage(playerid,-1," คุณต้องอยู่บนมอไซต์ส่งพิซซ่า!");
	    }
	    else return SendClientMessage(playerid,-1," คุณต้องเริ่มงาน part-time ก่อนใช้คำสั่งนี้!");
	}
	else return SendClientMessage(playerid,-1," คุณต้องอยู่ที่ Well Stacked Pizza Co. ในการที่จะใช้คำสั่งนี้!");
	return 1;
}
			    ...
			    for(new j=0;j<5;j++)
			    {
					...
					if(IsPlayerInRangeOfPoint(playerid,3.0,X,Y,Z))
					{
					    if(PizzaBikesPizzas[pizzabikes[j]] < 5)
					    {
							...
							SendClientMessage(playerid,-1," พิซซ่าถูกใส่ไว้ในมอไซต์!");
							ApplyAnimation(playerid, "INT_HOUSE", "wash_up",4.1,0,0,0,0,0,1);
					    	return 1;
						}
						else
						{
						    SendClientMessage(playerid,-1," มอไซต์คันนี้มีพิซซ่าครบ 5 กล่องแล้ว!");
						    return 1;
						}
					}
				}
			}
			else return SendClientMessage(playerid,-1," คุณต้องยืนอยู่บนพื้นในขณะที่ใช้คำสั่ง!");
		}
		else return SendClientMessage(playerid,-1," คุณไม่ได้ถือกล่องพิซซ่าอยู่!");
	}
	return 1;
}
...
CMD:getpizza(playerid, params[])
{
	if(IsInJob[playerid] == 1)
	{
	        ...
	        if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
	        {
			    ...
			    for(new j=0;j<5;j++)
			    {
					...
					if(IsPlayerInRangeOfPoint(playerid,3.0,X,Y,Z))
					{
						...
						{
						    SendClientMessage(playerid,-1," มอไซต์คันนี้ไม่เหลือกล่องพิซซ่าแล้ว!");
						    return 1;
						}
					}
				}
		...
		else return SendClientMessage(playerid,-1," คุณกำลังถือกล่องพิซซ่าอยู่!");
	}
	return 1;
}
	...
	if(IsPlayerInRangeOfPoint(playerid,3.0,2104.2502,-1806.3750,13.5547))
	{
	    if(IsInJob[playerid] == 1)
	    {
	        if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	        {
				...
				format(string,sizeof(string)," คุณได้รับ {7FFF00}%d$ {FFFFFF}สำหรับงาน part-time เด็กส่งพิซซ่า!",PlayerEarnings[playerid]+PlayerTips[playerid]);
			...
			else return SendClientMessage(playerid,-1," คุณต้องอยู่บนมอไซต์ส่งพิซซ่าเพื่อเลิกงานนี้!");
	    }
	    else return SendClientMessage(playerid,-1," คุณต้องเริ่มทำงานส่งพิซซ่าก่อนจะเลิกงาน!");
	}
	else return SendClientMessage(playerid,-1," คุณต้องอยู่หน้า Well Stacked Pizza Co. เพื่อเลิกงาน!");
	return 1;
}
...
CMD:startjob(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid,3.0,2104.2502,-1806.3750,13.5547))
	{
	    ...
	    else return SendClientMessage(playerid,-1," คุณได้ทำงาน part-time เป็นเด็กส่งพิซซ่า!");
	}
	else return SendClientMessage(playerid,-1," คุณต้องอยู่หน้า Well Stacked Pizza Co. เพื่อเริ่มงาน!");
	return 1;
}
...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
	if(oldstate == PLAYER_STATE_ONFOOT && newstate == PLAYER_STATE_DRIVER)
	{
		...
		if(vehid == pizzabikes[0] || vehid == pizzabikes[1] || vehid == pizzabikes[2] || vehid == pizzabikes[3] || vehid == pizzabikes[4])
		{
			...
			{
				SendClientMessage(playerid,-1," คุณต้องเริ่มงานก่อนที่จะขับมอไซต์ส่งพิซซ่า!");
				RemovePlayerFromVehicle(playerid);
			}
		}
	}
...
hook OnPlayerEnterCheckpoint(playerid)
{
	if(PlayerCheckpoint[playerid] == PIZZA_CHECKPOINT)
	{
	    if(IsPlayerAttachedObjectSlotUsed(playerid,PIZZA_INDEX))
	    {
			...
			format(string,sizeof(string)," คุณได้รับ {7FFF00}%d$ {FFFFFF}สำหรับการส่งพิซซ่า!",PAY_PER_CHECKPOINT);
			SendClientMessage(playerid,-1,string);
			RemovePlayerAttachedObject(playerid,PIZZA_INDEX);
			PlayerCheckpoint[playerid]=CHECKPOINT_NONE;
			DisablePlayerCheckpoint(playerid);
			...
			if(TipTime[playerid] > 0)
	        {
	            ...
	            format(string,sizeof(string)," ยินดีด้วย! คุณได้รับทิป {7FFF00}%d$ {FFFFFF}สำหรับการส่งอย่างรวดเร็ว!",tip);
	            SendClientMessage(playerid,-1,string);
	            PlayerPlaySound(playerid,1139,0.0,0.0,0.0);
	            HideTipTimeText(playerid);
			}
	    }
	    else return SendClientMessage(playerid,-1," คุณต้องถือกล่องพิซซ่าก่อนเดินเข้าจุด {FF0000}checkpoint{FFFFFF}!");
	}
	return 1;
}
