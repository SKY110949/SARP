...
hook OP_PickUpDynamicPickup(playerid, pickupid)
{
	if(pickupid == FarmerPickup) {
        ...
        CreateDynamic3DTextLabel("ผู้ช่วยชาวไร่\nพิมพ์ /farmerjob เพื่อรับงานนี้", -1, FARMER_POS_X,FARMER_POS_Y,FARMER_POS_Z, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 0);
		return 1;
	}
    return 1;
}
                ...
                if(far_veh[playerid] == vehicleid) {
                    if (IsPlayerInRangeOfPoint(playerid, 5.0, -53.5525,70.3079,4.0933) || IsPlayerInRangeOfPoint(playerid, 5.0, -377.8374,-1433.8853,25.7266)) {
                        ...
                        if (gettime() - gLastCheckpointTime[playerid] <= 5) {
                            ...
                            Log(anticheatlog, INFO, "%s ส่ง Farmer ในเวลา %d", ReturnPlayerName(playerid), gettime() - gLastCheckpointTime[playerid]);
                        ...
                        Log(paychecklog, INFO, "%s ได้รับ Paycheck %d จาก Farmer", ReturnPlayerName(playerid), randmoney);
