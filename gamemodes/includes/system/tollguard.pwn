...
#define L_sz_TollStringLocked ("ด่านเก็บค่าผ่านทาง: ขออภัยที่กั้นปิดชั่วคราว โปรดมาใหม่ในภายหลัง")
#define L_sz_TollStringNoMoney ("คุณมีเงินไม่เพียงพอที่จะผ่าน")
#define L_sz_TollStringBye ("ด่านเก็บค่าผ่านทาง: ขอบคุณ ขอให้ขับขี่ปลอดภัย")
...
CMD:tolls(playerid, params[])
{
	...
	if (Faction_GetTypeID(playerid) != FACTION_TYPE_POLICE) 
        return SendClientMessage(playerid, COLOR_GRAD1, "คุณไม่ใช่เจ้าหน้าที่ตำรวจ");
	...
	if(!strcmp(option, "lock", true))
	{
		...
		SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: %s %s ได้ปิดด่านกั้นทั้งหมด ! **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
	}
	else if(!strcmp(option, "release", true))
	{
		...
		SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: %s %s ได้เปิดด่านกั้นทั้งหมด ! **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
	}
	else if(!strcmp(option, "flint", true))
	{
		if(aTolls[FlintToll][E_tLocked] == 0)
		{
			aTolls[FlintToll][E_tLocked] = 1;
			Toll_CloseToll(FlintToll);
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Flint County ถูก \"ล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
		else
		{
			aTolls[FlintToll][E_tLocked] = 0;
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Flint County ถูก \"ปลดล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
	}
	else if(!strcmp(option, "richman", true))
	{
		if(aTolls[RichmanToll][E_tLocked] == 0)
		{
			aTolls[RichmanToll][E_tLocked] = 1;
			Toll_CloseToll(RichmanToll);
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Richman ถูก \"ล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
		else
		{
			aTolls[RichmanToll][E_tLocked] = 0;
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Richman ถูก \"ปลดล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
	}
	else if(!strcmp(option, "lv", true))
	{
		if(aTolls[LVToll][E_tLocked] == 0)
		{
			...
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ LS-LV ถูก \"ล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
		else
		{
			aTolls[LVToll][E_tLocked] = 0;
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ LS-LV ถูก \"ปลดล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
	}
	else if(!strcmp(option, "blueberryr", true))
	{
		if(aTolls[BlueberryTollR][E_tLocked] == 0)
		{
			aTolls[BlueberryTollR][E_tLocked] = 1;
			Toll_CloseToll(BlueberryTollR);
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Blueberry(ขวา) ถูก \"ล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
		else
		{
			aTolls[BlueberryTollR][E_tLocked] = 0;
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Blueberry(ขวา) ถูก \"ปลดล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
		}
	}
	else if(!strcmp(option, "blueberryl", true))
	{
		if(aTolls[BlueberryTollL][E_tLocked] == 0)//23914
		{
			aTolls[BlueberryTollL][E_tLocked] = 1;
			Toll_CloseToll(BlueberryTollL);
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Blueberry(ซ้าย) ถูก \"ล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
			
		}
		else
		{
			aTolls[BlueberryTollL][E_tLocked] = 0;
			SendFactionTypeMessage(FACTION_TYPE_POLICE, COLOR_RADIO, "** HQ Announcement: ด่านกั้นที่ Blueberry(ซ้าย) ถูก \"ปลดล็อก\" โดย %s %s **", Faction_GetRankName(factionid, playerData[playerid][pFactionRank]), ReturnRealName(playerid));
...
CMD:opentoll(playerid, params[])
{
    ...
    if(!playerData[playerid][pOnDuty])
	{
		...
		SendNearbyMessage(playerid, 20.0, COLOR_PURPLE, "%s จ่าย %d$ ให้กับด่านเก็บค่าผ่านทาง", ReturnRealName(playerid), TollCost);
	}
...
hook OP_PickUpDynamicPickup(playerid, pickupid) {
    if(pickupid == L_a_Pickup[0] || pickupid == L_a_Pickup[1] || pickupid == L_a_Pickup[2] || pickupid == L_a_Pickup[3] || pickupid == L_a_Pickup[4] || pickupid == L_a_Pickup[5] || pickupid == L_a_Pickup[6] || pickupid == L_a_Pickup[7] || pickupid == L_a_Pickup[8] || pickupid == L_a_Pickup[9])
	{
		if(!playerData[playerid][pOnDuty])
		{
		    SendClientMessage(playerid, COLOR_WHITE, "ด่านเก็บค่าผ่านทาง: สวัสดีเจ้าหน้าที่ คุณต้องการจะผ่านใช่ไหม?");
			SendClientMessage(playerid, COLOR_DARKGOLDENROD, "ใช้ \"/opentoll\" เพื่อเปิดด่านกั้น");
			return 1;
		}
		SendClientMessageEx(playerid, COLOR_WHITE, "ด่านเก็บค่าผ่านทาง: สวัสดี กรุณาจ่าย %d ดอลล่าสำหรับค่าผ่านทาง", TollCost);
