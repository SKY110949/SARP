...
ShowStats(playerid,targetid)
{
	...
	SendClientMessageEx(playerid, COLOR_LIGHTCYAN2, "| ค่าพลังงาน | ความหิว:[%d] ความกระหายน้ำ:[%d]", playerData[targetid][pHungry], playerData[targetid][pThirst]);
...
CMD:ooc(playerid, params[])
{
	...
	if (strlen(params) > 80) {
	    if(playerData[playerid][pAdmin]) {
			SendOOCMessage(COLOR_GRAD1, "(( [{ED3CCA}ทั่วโลก{B4B5B7}] {%06x}%s"EMBED_GRAD1"(%d): %.80s ))", GetPlayerColor(playerid) >>> 8, ReturnPlayerName(playerid), playerid, params);
			SendOOCMessage(COLOR_GRAD1, "... %s", params[80]);
		}
	    else if(playerData[playerid][pDonateRank]) {
			SendOOCMessage(COLOR_GRAD1, "(( [ทั่วโลก] {7535FF}%s{B4B5B7} {%06x}%s"EMBED_GRAD1"(%d): %.80s ))", Donate_GetName(playerData[playerid][pDonateRank]), GetPlayerColor(playerid) >>> 8, ReturnPlayerName(playerid), playerid, params);
			SendOOCMessage(COLOR_GRAD1, "... %s", params[80]);
		}
		else {
...
CMD:respawnme(playerid)
{
	...
	if(gInjuredTime[playerid] <= 0)
	{
		...
		SendClientMessage(playerid, COLOR_YELLOW, "คุณได้เสียเงินเพิ่มจำนวน $5,000 โดยเป็นค่ารักษาของโรงพยาบาล All Saints General Hospital");
	}
